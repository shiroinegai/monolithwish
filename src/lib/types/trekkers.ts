...
export const AttackTypeSchema = v.pipe(v.string(), v.picklist(['Melee', 'Ranged']));

export type AttackType = v.InferOutput<typeof AttackTypeSchema>;

export const PositionSchema = v.pipe(v.string(), v.picklist(['Vanguard', 'Versatile', 'Support']));
...
export const DamageTypeSchema = v.pipe(
	v.string(),
	v.picklist(['Auto Attack', 'Mark', 'Minion', 'Projectile', 'Skill', 'Ultimate'])
);

export type DamageType = v.InferOutput<typeof DamageTypeSchema>;

export const PotentialRaritySchema = v.pipe(v.string(), v.picklist(['common', 'core', 'rare']));
